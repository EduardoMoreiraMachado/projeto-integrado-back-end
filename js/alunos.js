...
//retorna os alunos pela sigla de um curso
const getCursoAlunos = function (filtro) {

    let sigla = filtro.toUpperCase();
    let listaAluno = [];
    let erro = true;

    //percorre o array principal de alunos
    alunos.forEach(item => {

        item.curso.forEach(element => {
            
            //localiza a palavra chave dentro do nome completo do aluno (usando o includes)
...
//retorna as disciplinas do curso do aluno pela matricula
const getDisciplinasAlunos = function (filtro) {
    ...
    alunos.forEach(item => {
        ...
        //localiza a matricula dentro do array principal de alunos (usando o includes)
        if(item.matricula.toLowerCase().includes(matricula)) {

            //adiciona no array do aluno um JSON com todos os elementos solicitados
            infoAluno.push(
                {
                    foto: item.foto,
                    nome: item.nome,
                    ...
                    sexo: item.sexo,
                    status: item.status,
                    curso: item.curso[0].sigla
                }
            );
        
            //percorre o array 'disciplinas' dentro do array de cursos dentro do array principal de alunos
            item.curso[0].disciplinas.forEach(element => {

                //adiciona no array de disciplinas um JSON com todos os elementos solicitados
                infoAluno.push(
                    {
                        nome: element.sigla,
                        carga: element.carga,
...
//retorna a lista de alunos pelo ano de conclusao
const getAlunosAno = function (filtro, curso) {

    let ano = filtro;
    let sigla = curso.toLowerCase();
    let listaAlunos = [];
    let erro = true;
    ...
    alunos.forEach(item => {
        ...
        item.curso.forEach(element => {
            ...
            if(element.conclusao.includes(ano) && item.curso[0].sigla.toLowerCase().includes(sigla)) {
...
//retorna a lista de alunos pelo status
const getAlunosStatus = function (filtro, curso) {

    let status = filtro.toLowerCase();
    let sigla = curso.toLowerCase();
    let listaAlunos = [];
    let erro = true;
    ...
    alunos.forEach(item => {
        ...
        //localiza as palavras chave dentro do status do aluno (cursando ou finalizado)
        if(item.status.toLowerCase().includes(status) && item.curso[0].sigla.toLowerCase().includes(sigla)) {
